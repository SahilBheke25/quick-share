...
import { useGetUserProfileByIdQuery } from "../redux/slice/slice";
...
const EquipmentComponent = () => {
  const userID: number | undefined = 20
  const {data, isLoading, error} = useGetUserProfileByIdQuery(userID, {
    skip: !userID, // This prevents the query from running if userID is undefined
  });
  ...
  if (error) return <h3>Error fetching owner data</h3>;
